...
Para iterar colecciones, Java introduce el método `forEach` como una alternativa declarativa y funcional al tradicional bucle `for` o `while` imperativo. Mientras que en lenguajes procedurales como C el desarrollador debe gestionar explícitamente la inicialización, la condición de parada y el incremento de un contador de índices o de un puntero, `forEach` delega todo el control del flujo iterativo a la propia estructura de datos subyacente.

Este método de la API de colecciones espera recibir como argumento un objeto que implemente la interfaz funcional `Consumer`. Dicha interfaz representa una operación que recibe un único argumento y no devuelve ningún resultado, siendo el equivalente conceptual más cercano a pasar un puntero a una función de tipo `void` en C. En la práctica moderna de Java, se proporciona directamente una expresión lambda en línea que contiene la lógica a aplicar.

A continuación, se muestra la implementación solicitada. La lista procesa cada entero internamente y la expresión lambda aplica la lógica condicional, mostrando el resultado por la salida estándar sin necesidad de gestionar la posición actual en la colección:
...
La decisión arquitectónica de definir la firma como `Consumer<? super T>` en lugar de exigir estrictamente `Consumer<T>` radica en la necesidad de maximizar la flexibilidad y usabilidad del método a través de la contravarianza. Si una lista contiene elementos de tipo `Integer` y la firma fuera estricta, solo se admitiría un consumidor exclusivo para `Integer`. Al usar el comodín `? super T`, el compilador permite que la colección sea procesada por un consumidor diseñado para operar sobre clases superiores en la jerarquía, como un `Consumer<Number>` o un `Consumer<Object>`, lo cual es un comportamiento matemáticamente seguro.

Este diseño obedece al principio estandarizado **PECS**, un acrónimo fundamental en la ingeniería de software en Java que significa *"Producer Extends, Consumer Super"*. Esta regla de diseño establece que, cuando una estructura o método actúa como **productor** (provee o devuelve datos al programa), su firma genérica debe emplear `? extends T` para asegurar que el dato entregado sea un subtipo válido. Inversamente, si actúa como **consumidor** (recibe o procesa datos), se debe usar `? super T` para garantizar que el procesador sea capaz de digerir el tipo suministrado o cualquiera de sus ancestros estructurales.

Aplicando el principio PECS al diseño de un método utilitario genérico `transformar(List<T> origen, Function transformador)`, se debe analizar el flujo de los datos. La función transformadora actúa consumiendo elementos de tipo `T` y produciendo nuevos elementos de tipo `R`. Para que la firma sea perfecta y admita la máxima reutilización de código (por ejemplo, permitir que una función que opera sobre `Object` transforme una lista de `String`), la definición rigurosa de los comodines genéricos debería establecerse como `Function<? super T, ? extends R>`.

## 16. Referencias a métodos. Podemos obtener una referencia a métodos de objetos o clases. Pon un ejemplo en JavaScript y en Java, de una clase `Persona` con un método `saludar`. En el código principal, crea una `Persona` con un nombre, y obtén una referencia a su método `saludar` en una variable local. Invoca `saludar` con esa referencia a su método `saludar`.

### Respuesta

Las referencias a métodos constituyen una característica del paradigma funcional que permite tratar las funciones, ya sean estáticas o pertenecientes a una instancia, como valores encapsulados. Estos valores pueden ser asignados a variables, pasados como parámetros o devueltos desde otras rutinas. Conceptualmente, suponen la evolución natural, robusta y orientada a objetos de los clásicos punteros a función presentes en el lenguaje C.

En lenguajes interpretados y dinámicos como JavaScript, las funciones son entidades de primera clase por naturaleza. Extraer un método de un objeto es tan directo como asignar el nombre de su propiedad a una variable. Sin embargo, al desvincular la función de su objeto contenedor, se pierde la referencia interna al contexto (`this`), por lo que es imperativo aplicar mecanismos como la función `bind()` para anclar la ejecución a la instancia de memoria original.

En Java, un lenguaje de tipado estático estricto, la obtención de la referencia se realiza mediante el operador de doble dos puntos (`::`). El sistema de tipos no genera un puntero libre en la memoria, sino que envuelve automáticamente el enlace subyacente en una instancia temporal que cumple con una interfaz funcional compatible. Como el método no recibe argumentos ni retorna valores, la interfaz diana idónea para almacenar la referencia es `Runnable`.
...
class PersonaJS {
    ...
    saludar() {
        console.log("Saludos cordiales desde JS, soy " + this.nombre);
    }
}

const personaJS = new PersonaJS("Ada Lovelace");
// Se obtiene la referencia forzando el anclaje del contexto 'this'
const referenciaSaludarJS = personaJS.saludar.bind(personaJS);

// Invocación a través de la referencia almacenada
referenciaSaludarJS(); 
...
class PersonaJava {
    ...
    public void saludar() {
        System.out.println("Saludos cordiales desde Java, soy " + nombre);
    }
}

public class MainReferencias {
    public static void main(String[] args) {
        PersonaJava personaJava = new PersonaJava("Alan Turing");
        
        // Se obtiene la referencia utilizando el operador :: y se guarda en una interfaz
        Runnable referenciaSaludarJava = personaJava::saludar;
        
        // Invocación ejecutando el método abstracto de la interfaz funcional
...
El ecosistema de la Máquina Virtual de Java tipifica el uso del operador de resolución de ámbito (`::`) en cuatro modalidades arquitectónicas diferentes. Esta clasificación se establece en función de la naturaleza intrínseca de la subrutina que se desea enlazar y del modo en que se pretende vincular con el objeto receptor en el momento de la futura ejecución.

La primera variante es la referencia a un **método estático** (`Clase::metodoEstatico`), aplicable a funciones globales que operan de forma independiente al estado de los objetos. La segunda es la referencia a un **constructor** (`Clase::new`), la cual se emplea frecuentemente en patrones de diseño como factorías para postergar la creación en memoria de nuevas entidades hasta que la interfaz funcional correspondiente sea invocada.

Las referencias sobre rutinas no estáticas se subdividen en dos enfoques de vinculación. Se permite apuntar a un **método de una instancia concreta** (`objeto::metodo`), donde la función queda atada irremediablemente a un bloque de memoria previamente inicializado. Como alternativa más dinámica, existe la referencia a un **método de instancia de un objeto arbitrario** (`Clase::metodoDeInstancia`). En este último diseño, la identidad del objeto receptor no se fija al compilar; el código pospone dicha decisión exigiendo que la instancia concreta se suministre como el primer parámetro en el instante de la llamada funcional.
...
public class TiposReferencias {
    public static void main(String[] args) {
        // 1. Referencia a método estático (recibe un double, devuelve un double)
        Function<Double, Double> refEstatica = Math::sqrt;

        // 2. Referencia a un constructor (no recibe nada, devuelve un ArrayList nuevo)
        Supplier<ArrayList<String>> refConstructor = ArrayList::new;

        // 3. Referencia a método de una instancia concreta
        String prefijoExistente = "Log del sistema: ";
        // Ancla la llamada 'concat' específicamente al objeto 'prefijoExistente'
        Function<String, String> refConcreta = prefijoExistente::concat; 

        // 4. Referencia a método de instancia sobre cualquier instancia arbitraria
        // No hay un objeto predefinido; se requerirá proporcionar un String al invocarlo
        Function<String, String> refArbitraria = String::toUpperCase;
    }
}
...
La ordenación de colecciones heterogéneas conforma uno de los ámbitos donde la transición al paradigma funcional ha purgado más líneas de código burocrático. En lugar de redactar clases completas que implementen interfaces, o de estructurar complejas funciones externas mediante punteros (como requiere la función `qsort` de la librería estándar de C), el desarrollador inyecta un bloque de comportamiento lambda que cumple con la rúbrica de comparación.

En la primera versión implementada, el algoritmo se diseña manualmente en el interior del bloque lambda. Este bloque asimila dos objetos extraídos de la lista y se encarga de retornar valores numéricos negativos, positivos o cero. Para construir el criterio compuesto, se ejecuta inicialmente una comparación matemática sobre el atributo de la edad. Únicamente en el caso de detectarse una igualdad (retorno de valor cero), se deriva el flujo hacia una segunda evaluación utilizando el orden lexicográfico nativo de las cadenas de texto del nombre.

La segunda versión demuestra el inmenso poder compositivo de las factorías añadidas a las interfaces funcionales modernas. Prescindiendo por completo de la lógica condicional, el programador encadena referencias a los métodos extractores (`getEdad`, `getNombre`) a través de las funciones estáticas `comparingInt` y `thenComparing` proporcionadas por la interfaz `Comparator`. El sistema sintetiza internamente la matemática de evaluación, produciendo una instrucción altamente descriptiva, robusta e inmune a los habituales errores de polaridad al restar valores numéricos.
...
public class OrdenacionFuncional {
    public static void main(String[] args) {
        List<Persona> personasV1 = Arrays.asList(
            new Persona("Carlos", 30), new Persona("Ana", 25), new Persona("Beatriz", 30)
        );
        List<Persona> personasV2 = Arrays.asList(
            new Persona("Carlos", 30), new Persona("Ana", 25), new Persona("Beatriz", 30)
        );

        // --- VERSIÓN 1: Expresión lambda con lógica condicional manual ---
        Collections.sort(personasV1, (p1, p2) -> {
            int comparacionPrimaria = Integer.compare(p1.getEdad(), p2.getEdad());
            if (comparacionPrimaria == 0) {
                // Desempate lexicográfico secundario
                return p1.getNombre().compareTo(p2.getNombre());
            }
            return comparacionPrimaria;
        });

        // --- VERSIÓN 2: Enfoque declarativo componiendo referencias de método ---
        Collections.sort(personasV2, 
